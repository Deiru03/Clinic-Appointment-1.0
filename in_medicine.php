...
<div class="container-fluid">
	<form action="" id="in_medicine">
		...
		<div class="form-group">
			...
			<input type="hidden" class="form-control form-control-sm" name="med" id="med" value="<?php echo isset($medicine) ? $medicine : '' ?>" >
		</div>
        <div class="form-group">
		...
		<div class="form-group">
			<label for="description" class="control-label">Quantity to be Add</label>
			<input type="number" class="form-control form-control-sm" name="quan" id="quan" onchange="sum()" required>
...
<script>
	$(document).ready(function(){
		$('#in_medicine').submit(function(e){
			...
			$('#msg').html('')
			$.ajax({
				url:'ajax.php?action=in_medicine',
				method:'POST',
				data:$(this).serialize(),
				success:function(resp){
					if(resp == 1){
						alert_toast("Quantity successfully added.","success");
						setTimeout(function(){
							location.reload()	
						},1750)
					}else if(resp == 2){
